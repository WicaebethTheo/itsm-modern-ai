  ...
  return (
    <div className="flex max-h-80 flex-col overflow-auto">
      {items.map((it, i) => (
        <label
          key={it.ext_id}
          className={`flex cursor-pointer items-center gap-2.5 px-4 py-2 text-[12.5px] ${i < items.length - 1 ? "border-b border-border/50" : ""}`}
        >
          <input
            type="checkbox"
            className="h-4 w-4"
...
export function Scope() {
  ...
  async function save() {
    ...
    try {
      ...
      setMsg({ kind: "success", text: t("Périmètre enregistré.", "Scope saved.") });
    } catch (e: unknown) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <Card className="overflow-hidden">
          <PanelHead
            ...
            subtitle={`${ents.size} ${t("sélectionnée(s)", "selected")}`}
          />
          <CheckList
            items={entities.data ?? []}
            selected={ents}
      ...
      <Button onClick={save}>{t("Enregistrer le périmètre", "Save scope")}</Button>
    </div>
  );
}
