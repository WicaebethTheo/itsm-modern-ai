...
⚠️ Si AUCUN mot de passe admin n'est configuré, les endpoints d'admin sont OUVERTS
(pilote sur réseau interne non exposé) avec un avertissement. À durcir avant un
déploiement payant (PRD §12). 2FA TOTP : non implémenté (OFF par défaut, PRD).
"""
...
def require_auth(request: Request) -> None:
    ...
    Ouvert si aucun mot de passe admin n'est configuré (pilote). Sinon, session requise.
    """
    ...
    if not configured:
        logger.warning("accès admin non protégé : aucun ADMIN_PASSWORD configuré (pilote)")
        return
