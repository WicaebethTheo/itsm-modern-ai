...
- Identifiants = **email + mot de passe**. Le mot de passe est haché avec **Argon2**, le hash lui-même chiffré (Fernet) en base ; gestion par session. L'adresse est stockée en clair sous une clé **réservée**, hors `/api/config` — elle n'est **jamais** renvoyée par une route publique (la diffuser à un anonyme lui offrirait la moitié du couple à deviner).
- **Le compte se crée à la première visite** (`POST /api/auth/setup`), plus par une variable d'environnement. La route est **publique** — par construction, il n'existe aucun identifiant pour l'atteindre — mais **fail-closed** : dès qu'un compte existe, elle répond **409** sans rien modifier. Elle est comptée par le **même rate-limit que le login**.
