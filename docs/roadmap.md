...
- **Portage PostgreSQL** — ✅ livré (`docs/postgresql.md`). Driver `psycopg`, service
  `postgres` à part entière des composes, migrations Alembic, ORM et suite de tests validés
  sur PostgreSQL 17 réel (la majeure des composes, du `postgresql-client-17` de l'image et
  de la CI, les trois bougeant ensemble). Reste à éprouver en prod : HA, gros volumes.
...
- **Modules Supporter** (open-core) — multi-tenant, SSO SAML, audit log signé ISO, self-service AD hors-bande. Hors cible PME, et hors du catalogue actuel : `KNOWN_FEATURES` (`domain/licensing.py`) ne connaît que `pii_advanced`, `multi_entity` et `scheduled_exports`, et toute autre clé portée par une licence est silencieusement ignorée. Les livrer supposerait donc d'abord d'étendre le catalogue ; aucune clé ne les débloquerait aujourd'hui.
