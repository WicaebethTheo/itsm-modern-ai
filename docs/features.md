...
| **Congés & remplaçants** | Un technicien absent **sort du périmètre effectif** : jamais proposé, donc jamais assigné — plutôt que corrigé après coup, ce qui créerait une divergence entre le Journal et GLPI. Son remplaçant **hérite de ses domaines** dans le prompt (sans quoi le modèle lui attribuerait des tickets qu'il est décrit comme ne couvrant pas → confiance basse → « à trier »). Bornes **incluses**, granularité jour, évaluées dans le **fuseau local configuré**. L'absence **expire d'elle-même**. |
...
| **Masquage PII configurable** | Email / téléphone / IBAN / mot de passe activables motif par motif, avec avertissement DPO si on en désactive un. **Tous à `true` par défaut dans les réglages, mais en Community le moteur force IBAN, secrets et IP/MAC à *inactif*** quel que soit le réglage : seuls email et téléphone sont réellement masqués. IBAN/secrets/IP-MAC/NIR-SIRET = features **Supporter** (déverrouillées par licence). |
...
| **Cost cap glissant + page Coûts & quotas** | Plafond € / jour configurable (défaut 5 €) — au-delà, les Tickets restent « à trier » sans appel facturant. Le plafond se règle sur **Moteur › Garde-fous** ; la page **Coûts & quotas** (`/cost`) est en **lecture seule** : dépense 24 h vs plafond (jauge), appels journalisés et tarifs. Les tarifs, eux, ne sont éditables depuis aucun écran — ils viennent de l'environnement (`LLM_PRICE_*_PER_MTOK`) ou d'un `POST /api/config`. |
| **Bac à sable** | `POST /api/sandbox` permet de tester un texte de ticket sans toucher GLPI. UI dédiée (entrée **Bac à sable**, section Avancé) affiche la décision simulée + résolution des noms. |
