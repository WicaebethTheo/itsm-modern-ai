...
> Déploiement pilote : deux conteneurs Docker, le moteur et sa base PostgreSQL, seule base
> supportée. Sans haute disponibilité : un seul cluster, pas de réplication, pas de bascule.
> Ce déploiement n'est pas une architecture de production. Voir la note « durcissement »
...
- ~1,5 Gio de RAM pour la stack et ~2 Gio de disque. Détail : la base réserve 256 Mio
  (le moteur 128 Mio) et est plafonnée à 1 Gio (le moteur à 512 Mio). Dimensionnez sur les
  plafonds pour ne pas les toucher.
...
À faire avant toute mise à jour, et régulièrement. Une sauvegarde complète comprend deux
...
en marche produit un cluster incohérent, fichiers de données et WAL étant capturés à des
...
> de la simplicité : l'un comme l'autre auraient réintroduit un secret à transporter, ce que
...
PostgreSQL étant la seule base supportée, cela demande un réseau, deux volumes et deux
conteneurs, pas une commande unique. Si vous n'avez pas de raison précise d'éviter compose,
...
- **L'ordre n'a pas d'importance.** `docker run` n'a pas d'équivalent de `depends_on` :
  ...
  bout du plafond il sort en erreur, avec la dernière erreur de connexion, plutôt que de
  rester bloqué en silence.
...
- **`FOWNER` en plus côté base** : l'entrypoint officiel de PostgreSQL ajuste les droits du
  PGDATA au premier boot. Pas de `--read-only` non plus sur ce conteneur, car PostgreSQL écrit
...
s'ouvre dans la foulée, sans reconnexion. Aux accès suivants, c'est l'écran de
connexion habituel (email + mot de passe). Toute la configuration se fait ensuite ici,
aucun fichier n'est à éditer :
...
- **Périmètre effectif** (scan GLPI puis sélection, sur trois écrans) : lancez d'abord le scan GLPI (`POST /api/glpi/sync`), qui met en cache catégories, entités, techniciens et groupes. La sélection de ce que l'IA a le droit d'utiliser se fait ensuite sur trois écrans distincts de Configuration :
  ...
  Le moteur n'agit que dans ce périmètre effectif. Il n'y a plus de fichier YAML : les fiches sont éditées dans l'UI et stockées en base.
...
L'adresse et le nom affiché, eux, ne se changent pas depuis cette page, mais avec
...
email. Le produit ne parle à aucun serveur SMTP, par contrainte de souveraineté assumée.
Le seul chemin de récupération, quand on ne peut plus se connecter et que la page
Compte & sécurité est hors de portée, est la CLI livrée dans l'image. Autrement dit,
l'accès shell à la machine hôte est le facteur d'authentification de dernier recours
(quiconque l'a pouvait déjà lire `master.key` dans le volume : cette CLI n'élargit pas la
surface d'attaque).
...
> façon masquée, pour ne pas laisser de copie en clair dans `docker inspect`,
...
`/health` est en échec si GLPI est injoignable (FR-27, FR-1), sans démarrage
silencieux dégradé. Il répond 503 si les secrets sont illisibles (master key
incohérente), avec un message qui nomme la cause.

> `/health/live` est ce que sondent les conteneurs, délibérément : `/health`
...
> 3. N'activez ces options que derrière un proxy fiable : sans cela, n'importe quel client peut forger l'IP via le header.
...
voulu, la majeure reste figée (cf. [`docs/postgresql.md`](postgresql.md#6-la-majeure-17-est-épinglée)).
...
qu'elle a écrit. Une archive du volume ne remplace pas ce dump, car copier `data/postgres` d'un
...
> plutôt que de générer une nouvelle clé par-dessus des secrets existants ; un arrêt
> explicite vaut mieux qu'une instance qui démarre « verte » avec des secrets illisibles et
...
Le retour arrière est entièrement automatisé, base comprise. Dans l'ordre : il demande une
confirmation tapée (voir ci-dessous), arrête le seul moteur (la base doit rester en marche
pour être restaurée), dumpe l'état actuel dans `data/pre-rollback-<horodatage>` — un rollback
raté reste donc réversible —, remet le schéma à plat, restaure ensemble le dump et la master
key (l'une sans l'autre est inutile), remet le code ou l'image de l'époque, préserve le port
publié, puis redémarre l'instance.

La confirmation demande de taper l'horodatage de la sauvegarde visée : `Entrée` ne vaut pas
oui, `--yes` non plus, et l'absence de terminal encore moins ; le script s'arrête plutôt que
d'inventer une réponse à une opération destructive. Pour restaurer sans terminal
(automatisation), déclarez-le explicitement :
...
Le script vérifie Docker, crée `.env`, génère la clé de chiffrement, build et démarre
...
> [`docs/postgresql.md`](postgresql.md#3-variables-denvironnement).
...
sauvegarde, et une instance actuellement à l'arrêt reste sauvegardable, le script démarrant la
...
Ce déploiement n'est pas une architecture de production : un seul cluster, pas de réplication,
pas de bascule automatique, pas de tuning, sauvegardes déclenchées par l'exploitant, et un
rate-limit de login toujours en mémoire (donc mono-process). C'est acceptable en pilote. Un
plan de durcissement est prévu avant tout déploiement payant (cf. PRD §12).

Le contrat d'exploitation, qu'il vaut mieux connaître avant l'incident plutôt qu'après :
deux services à superviser, deux volumes à sauvegarder, ~1,5 Gio de RAM, et une majeure
PostgreSQL épinglée qu'il faudra migrer un jour
(procédure : [`docs/postgresql.md`](postgresql.md#6-la-majeure-17-est-épinglée)).
La restauration complète, elle, est automatisée (`./install.sh --rollback`) et ne demande
